...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (username.trim() && password.trim()) {
      ...
      // For this simulation, we'll just create a user object.
      onLogin({
        username: username.trim(),
        name: 'Siswa Uji Coba', // Mock name
      });
    } else {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-200">
      <div className="w-full max-w-md p-8 space-y-8 bg-white rounded-lg shadow-2xl">
        <div className="text-center">
          ...
          />
          <h1 className="text-3xl font-bold text-black">Portal Siswa</h1>
          <p className="mt-2 text-black">Selamat Datang di Ujian Berbasis Komputer MIN SINGKAWANG</p>
        </div>
